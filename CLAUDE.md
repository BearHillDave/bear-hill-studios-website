...
- ⚠️ **`.nav.menu-open` must set `backdrop-filter: none`.** The `.nav-menu` overlay is `position: fixed` and a *child* of `.nav`. When the page is scrolled, `.nav.scrolled` applies `backdrop-filter: blur(14px)`, which makes `.nav` a containing block for the fixed overlay — collapsing it from the viewport to the nav's 88px height the moment you open the menu after scrolling. Clearing the filter while open removes the containing block (the overlay covers the bar anyway, so the blur isn't missed); the scrolled bar keeps its blur once the menu closes.
